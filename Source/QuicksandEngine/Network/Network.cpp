...
#define EXIT_ASSERT LOG_ASSERT(0);
...
bool ClientSocketManager::Connect()
{
	...
	RemoteEventSocket *pSocket = QSE_NEW RemoteEventSocket;
...
void GameServerListenSocket::VHandleInput() 
{
	...
	{
		RemoteEventSocket * sock = QSE_NEW RemoteEventSocket(new_sock, theipaddr);
		int sockId = g_pSocketManager->AddSocket(sock);
		int ipAddress = g_pSocketManager->GetIpAddress(sockId);
        shared_ptr<EvtData_Remote_Client> pEvent(QSE_NEW EvtData_Remote_Client(sockId, ipAddress));
...
void NetworkEventForwarder::ForwardEvent(IEventDataPtr pEventData)
{
	...
	shared_ptr<BinaryPacket> eventMsg(QSE_NEW BinaryPacket(out.rdbuf()->str(), (u_long)out.pcount()));
...
void NetworkGameView::NewActorDelegate(IEventDataPtr pEventData)
{
    ...
    if (pActor && pActor->GetType() == "Teapot")
    {
        ...
        {
            m_ActorId = actorId;
			shared_ptr<EvtData_Network_Player_Actor_Assignment> pEvent(QSE_NEW EvtData_Network_Player_Actor_Assignment(m_ActorId, m_SockId));
			IEventManager::Get()->VQueueEvent(pEvent);
        }
    }
}
