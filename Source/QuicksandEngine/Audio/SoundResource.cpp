...
unsigned int WaveResourceLoader::VGetLoadedResourceSize(char *rawBuffer, unsigned int rawSize)
{
	...
	while(file < fileEnd)
	{
		...
		switch(type)
		{
			...
			{
				LOG_ASSERT(false && "This wav file is compressed.  We don't handle compressed wav at this time");
				break;
			}
	...
	while(file < fileEnd)
	{
		...
		switch(type)
		{
			...
			case mmioFOURCC('d', 'a', 't', 'a'):
			{
				...
				{
					LOG_ASSERT(0 && _T("Wav resource size does not equal the buffer size"));
					return 0;
				}
...
//
int VorbisSeek(void* data_src, int64_t offset, int origin)            
{
	OggMemoryFile *pVorbisData = static_cast<OggMemoryFile *>(data_src);
	if (NULL == pVorbisData) 
	{
    ...
    default:
      LOG_ASSERT(false && "Bad parameter for 'origin', requires same as fseek.");
      break;
  };
...
unsigned int OggResourceLoader::VGetLoadedResourceSize(char *rawBuffer, unsigned int rawSize)
{
	...
	OggMemoryFile *vorbisMemoryFile = QSE_NEW OggMemoryFile; 
...
bool OggResourceLoader::ParseOgg(char *oggStream, size_t length, shared_ptr<ResHandle> handle)
{
	...
	{
		LOG_ASSERT(0 && _T("The Ogg size does not match the memory buffer size!"));
		ov_clear(&vf);
		SAFE_DELETE(vorbisMemoryFile);
		return false;
	}
