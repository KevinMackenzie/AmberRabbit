...
// PathingNode
//--------------------------------------------------------------------------------------------------------
QSE_MEMORYPOOL_DEFINITION(PathingNode);
QSE_MEMORYPOOL_AUTOINIT(PathingNode, 128);  // there are currently 81 pathing nodes generated; if you make more, you should increase this number
...
{
	LOG_ASSERT(pStartNode);
	LOG_ASSERT(pGoalNode);
...
PathPlanNode* AStar::AddToOpenSet(PathingNode* pNode, PathPlanNode* pPrevNode)
{
	...
	if (it == m_nodes.end())
	{
		pThisNode = QSE_NEW PathPlanNode(pNode,pPrevNode,m_pGoalNode);
		m_nodes.insert(std::make_pair(pNode,pThisNode));
	}
	else
	{
        LOG_WARNING("Adding existing PathPlanNode to open set");
		pThisNode = it->second;
...
void AStar::ReinsertNode(PathPlanNode* pNode)
{
    ...
    LOG_WARNING("Attemping to reinsert node that was never in the open list");
	InsertNode(pNode);
}
...
PathPlan* AStar::RebuildPath(PathPlanNode* pGoalNode)
{
	...
	PathPlan* pPlan = QSE_NEW PathPlan;
...
PathingNode* PathingGraph::FindRandomNode(void)
{
	...
	unsigned int node = QuicksandEngine::g_pApp->m_pGame->GetRNG().Random(numNodes);
...
void PathingGraph::LinkNodes(PathingNode* pNodeA, PathingNode* pNodeB)
{
	...
	PathingArc* pArc = QSE_NEW PathingArc;
...
{
	PathingGraph* pPathingGraph = QSE_NEW PathingGraph;
