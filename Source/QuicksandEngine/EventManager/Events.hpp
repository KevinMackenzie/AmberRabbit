#ifndef QSE_EVENTS_HPP
#define QSE_EVENTS_HPP
...
public:
    ...
    virtual void VSerialize(std::ostrstream& out) const
    {
        LOG_ERROR(GetName() + string(" should not be serialzied!"));
    }
...
public:
    ...
    explicit EvtData_Game_State(const BaseGameState gameState, const string &parameter)
	...
	const string GetParameter(void) const 
	{
		return m_parameter;
	}
};
...
public:
    ...
    explicit EvtData_Decompress_Request( std::wstring zipFileName, string filename  )
...
public:
    ...
    EvtData_Decompression_Progress( int progress, std::wstring zipFileName, string filename, void *buffer )
        : m_progress(progress),
        m_zipFileName( zipFileName ),
        m_fileName ( filename ),
        m_buffer (buffer)
...
{
    string m_soundResource;

public:
    static const EventType sk_EventType;

    EvtData_PlaySound(void) { }
    EvtData_PlaySound(const string& soundResource)
    ...
    const string& GetResource(void) const
    {
