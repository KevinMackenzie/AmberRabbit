#ifndef QSE_MEMORYMACROS_HPP
#define QSE_MEMORYMACROS_HPP

//---------------------------------------------------------------------------------------------------------------------
// These macros are designed to allow classes to easily take advantage of memory pools.  To use, follow this steps:
// 1) Call QSE_MEMORYPOOL_DECLARATION() in the class declaration
// 2) Call QSE_MEMORYPOOL_DEFINITION() in the cpp file
// 3) Call QSE_MEMORYPOOL_AUTOINIT() or QSE_MEMORYPOOL_AUTOINIT_DEBUGNAME() in the cpp file
// 
...
// 
// IMPORTANT: InitMemoryPool() and DestroyMemoryPool() must be called manually unless you use the QSE_MEMORYPOOL_AUTOINIT()
// macro below.
//---------------------------------------------------------------------------------------------------------------------
#define QSE_MEMORYPOOL_DECLARATION(__defaultNumChunks__) \
    public: \
...
//---------------------------------------------------------------------------------------------------------------------
#define QSE_MEMORYPOOL_DEFINITION(_className_) \
	...
	{ \
		...
		s_pMemoryPool = QSE_NEW MemoryPool; \
		if (debugName) \
	...
	{ \
		LOG_ASSERT(s_pMemoryPool != NULL); \
		SAFE_DELETE(s_pMemoryPool); \
	} \
    void* _className_::operator new(size_t size) \
    { \
        LOG_ASSERT(s_pMemoryPool); \
...
// pool yourself.
//---------------------------------------------------------------------------------------------------------------------
#define QSE_MEMORYPOOL_AUTOINIT_DEBUGNAME(_className_, _numChunks_, _debugName_) \
...
#define QSE_MEMORYPOOL_AUTOINIT(_className_, _numChunks_) QSE_MEMORYPOOL_AUTOINIT_DEBUGNAME(_className_, _numChunks_, #_className_)
