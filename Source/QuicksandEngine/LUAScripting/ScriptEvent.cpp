...
void ScriptEvent::RegisterEventTypeWithScript(const char* key, EventType type)
{
	...
	// error checking
	LOG_ASSERT(eventTypeTable.IsTable());
	LOG_ASSERT(eventTypeTable[key].IsNil());
...
{
	LOG_ASSERT(s_creationFunctions.find(type) == s_creationFunctions.end());
	LOG_ASSERT(pCreationFunctionPtr != NULL);
